...
params:
  ...
  log10_ks_cutoff: 0
  subGenomeFile: ../Data/outfile_processed_byHand.tab
output:
...
## Compared to Paper
How many chromosomes with synteny to sorghum were correctly placed in subgenome 1 or subgenome 2? Items in "Paper" only are missed subgenome assignments.  Items in "Greedy" only are false assignments.  Items in center are correct. (this is an unfair comparison if I sorted by hand)
...
Genes placed in the "correct" subgenome 
...
```{r GOTerms_by_EVCode, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=4, fig.height=4, fig.align='center'}
...
g <- draw.pairwise.venn(
  ...
  cross.area = intersect(goAnnotations.sub1 %>% select(gene2, "GO Term") %>% distinct(), 
                         converted %>% select(gene2, "GO Term") %>% distinct()
                         ) %>% nrow(),
  category = c("SubGenome 1", "SubGenome 2"), 
  lty = rep("blank", 2), 
  fill = c("light blue", "green"), 
...
g <- draw.pairwise.venn(
  ...
  area2 = converted %>% filter(grepl("EV-EXP", EVCode)) %>% nrow(),
  cross.area = intersect(goAnnotations.sub1 %>% filter(grepl("EV-EXP", EVCode)) %>% select(gene2, "GO Term") %>% distinct(), 
                         converted %>% filter(grepl("EV-EXP", EVCode)) %>% select(gene2, "GO Term") %>% distinct()
                         ) %>% nrow(), 
  category = c("SubGenome 1", "SubGenome 2"), 
  lty = rep("blank", 2), 
  fill = c("light blue", "green"), 
...
GO Terms by subgenome, only counting GO Terms that are in GOSlimPlant

```{r GOTerms_by_subgenome_goslim, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=4, fig.height=4}
# All EV-Codes, Unique GO Terms
grid.newpage()
g <- draw.pairwise.venn(
  area1 = goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term") %>% distinct() %>% nrow(),
  area2 = goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term") %>% distinct() %>% nrow(),
  cross.area = intersect(goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term"), 
                         goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term")) %>% 
              nrow(), 
  category = c("SubGenome 1", "SubGenome 2"), 
  lty = rep("blank", 2), 
  fill = c("light blue", "green"), 
...
grid.newpage()
g <- draw.pairwise.venn(
  area1 = goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term") %>% distinct() %>% nrow(),
  area2 = goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term") %>% distinct() %>% nrow(),
  cross.area = intersect(goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm"))%>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term"), 
                         goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term")
                         ) %>% nrow(), 
  category = c("SubGenome 1", "SubGenome 2"), 
  lty = rep("blank", 2), 
  fill = c("light blue", "green"), 
...
grid.arrange(gTree(children=g), top="GO Term by Subgenome\n(EV-EXP only, GOSlim only)")
```

GO Terms by subgenome, only counting GO Terms that are in GOSlimPlant and are Molecular Function terms.

```{r GOTerms_by_subgenome_goslim_MF, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=4, fig.height=4}
# All EV-Codes, Unique GO Terms
grid.newpage()
g <- draw.pairwise.venn(
  area1 = goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term") %>% 
    filter(`GO Term` %in% MFterms) %>% distinct() %>% nrow(),
  area2 = goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term") %>% 
    filter(`GO Term` %in% MFterms) %>% distinct() %>% nrow(),
  cross.area = intersect(goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term") %>% filter(`GO Term` %in% MFterms), 
                         goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% select("GO Term") %>% filter(`GO Term` %in% MFterms)) %>%
                          nrow(), 
  category = c("SubGenome 1", "SubGenome 2"), 
  lty = rep("blank", 2), 
  fill = c("light blue", "green"), 
...
grid.arrange(gTree(children=g), top="GO Term by Subgenome\n(All EV-Codes, GOSlim, MF only)")

#EXP only, Unique GO Terms
grid.newpage()
g <- draw.pairwise.venn(
  area1 = goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term") %>% 
    filter(`GO Term` %in% MFterms) %>% distinct() %>% nrow(),
  area2 = goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term") %>% 
    filter(`GO Term` %in% MFterms) %>% distinct() %>% nrow(),
  cross.area = intersect(goAnnotations.sub1 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm"))%>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term") %>% 
                           filter(`GO Term` %in% MFterms), 
                         goAnnotations.sub2 %>% inner_join(goSlim, by=c("GO Term" = "GOSlimTerm")) %>% filter(grepl("EV-EXP", EVCode)) %>% select("GO Term") %>% 
                           filter(`GO Term` %in% MFterms)
                         ) %>% nrow(), 
  category = c("SubGenome 1", "SubGenome 2"), 
...
grid.arrange(gTree(children=g), top="GO Term by Subgenome\n(EV-EXP, GOSlim, MF only)")
```

```{r aggregates, include=FALSE}
...
```{r diffExpression_subgenome, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=4, fig.height=4}
