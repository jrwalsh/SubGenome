...
# library(venneuler)
# i <- intersect(subgenome.chromosomes.test, subgenome.truth) %>% nrow()
# t <- setdiff(subgenome.chromosomes.test, subgenome.truth) %>% nrow()
# p <- setdiff(subgenome.truth, subgenome.chromosomes.test) %>% nrow()
# vd <- venneuler(c(Paper=p, Test=t, "Paper&Test"=i))
# vd$labels <- c(
#   paste("Paper\n",p+i),
#   paste("Test\n",t+i),
#   paste("Both\n",i)
...
```{r compareResultsToPaper, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=4, fig.height=3, fig.align='center'}
library(VennDiagram)
# i <- intersect(subgenome.chromosomes.test, subgenome.truth) %>% nrow()
# t <- setdiff(subgenome.chromosomes.test, subgenome.truth) %>% nrow()
# p <- setdiff(subgenome.truth, subgenome.chromosomes.test) %>% nrow()
