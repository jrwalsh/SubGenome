...
```{r chromosome_plot_exp, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=8, fig.height=6}
subgenome.positions <-
  subgenome %>%
...
```{r chromosome_plot_abundance, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=8, fig.height=6}
subgenome.positions <-
  subgenome %>%
  left_join(B73.v4.gene.positions, by=c("gene2"="geneID")) %>%
  select(Chromosome, Pos_start, Pos_end, subgenome) %>%
...
dominant_homeolog_abun <- expressedProteinsPairs
dominant_homeolog_abun$Subgenome <- "no dominance"
dominant_homeolog_abun$Subgenome[dominant_homeolog_abun$dNSAF_mean1 / dominant_homeolog_abun$dNSAF_mean2 > 2] <- "Dominant Abundance"
dominant_homeolog_abun$Subgenome[dominant_homeolog_abun$dNSAF_mean2 / dominant_homeolog_abun$dNSAF_mean1 > 2] <- "Dominant Abundance"
dominant_homeolog_abun$Maize1[dominant_homeolog_abun$dNSAF_mean2 / dominant_homeolog_abun$dNSAF_mean1 > 2] <- dominant_homeolog_abun$Maize2[dominant_homeolog_abun$dNSAF_mean2 / dominant_homeolog_abun$dNSAF_mean1 > 2]
df0 <- dominant_homeolog_abun[dominant_homeolog_abun$Subgenome %in% c("no dominance"),]
df0$Maize1 <- df0$Maize2
dominant_homeolog_abun <- bind_rows(dominant_homeolog_abun, df0)
dominant_homeolog_abun <-
  dominant_homeolog_abun %>%
  select(Maize1, Subgenome) %>%
  left_join(B73.v4.gene.positions, by=c("Maize1"="geneID")) %>%
  select(Chromosome, Pos_start, Pos_end, Subgenome)
...
segment_data$Subgenome[segment_data$Subgenome %in% c("sub2")] <- "Maize2"
# segment_data$Subgenome[segment_data$Subgenome %in% c("dom")] <- "Dominant Homeolog"
segment_data <- segment_data %>% subset(Subgenome %in% c("Maize1", "Maize2", "Dominant Abundance"))
centromere_data <- df1

## Make the plot
chromosome_plot_abundance <-
  ggplot(data=B73.v4.chr.size) +
  geom_bar(aes(B73.v4.chr.size$chromosome, B73.v4.chr.size$size), stat="identity", fill="grey70") +
  # geom_segment(data=segment_data, aes(x=Chromosome+x, xend=Chromosome+xend, y=Pos_start, yend=Pos_end, colour=Subgenome), size=.001) +
  geom_rect(data=segment_data, inherit.aes = F, aes(xmin=Chromosome+x, xmax=Chromosome+xend, ymin=Pos_start, ymax=Pos_end, color=Subgenome, fill=Subgenome)) +
...
```{r extra4, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=4, fig.height=4, fig.align='center'}
...
```{r extra6, echo=FALSE, warning=FALSE, message=FALSE, fig.width=4, fig.height=4, fig.align='center'}
...
# ggsave("Output/Figure 6.png", plot = subgenome_expression_protein_foldchange_scatter)
# ggsave("Output/Figure 7.png", plot = subgenome_expression_protein_scatter)
# ggsave("Output/Figure 8.png", plot = subgenome_expression_protein_scatter_rep)
