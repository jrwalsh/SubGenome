...
ggplot(data=df, aes(x=Type, y=Count)) +
  ...
  title="GO Term Assignments by Evidence Code"
)
```
...
grid.newpage()
g <- draw.pairwise.venn(
  area1 = goAnnotations.sub1 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm")) %>% filter(type=="MF") %>% select(goTerm) %>% 
    distinct() %>% nrow(),
  area2 = goAnnotations.sub2 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm")) %>% filter(type=="MF")  %>% select(goTerm) %>% 
    distinct() %>% nrow(),
  cross.area = intersect(goAnnotations.sub1 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm")) %>% filter(type=="MF") %>% select(goTerm), 
                         goAnnotations.sub2 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm")) %>% filter(type=="MF") %>% select(goTerm)) %>%
                          nrow(), 
  category = c("SubGenome 1", "SubGenome 2"), 
  lty = rep("blank", 2), 
  fill = c("light blue", "green"), 
...
grid.newpage()
g <- draw.pairwise.venn(
  area1 = goAnnotations.sub1 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm")) %>% filter(evCode=="EXP") %>% filter(type=="MF") %>% select(goTerm) %>%
    distinct() %>% nrow(),
  area2 = goAnnotations.sub2 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm")) %>% filter(evCode=="EXP") %>% filter(type=="MF") %>% select(goTerm) %>%
    distinct() %>% nrow(),
  cross.area = intersect(goAnnotations.sub1 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm"))%>% filter(evCode=="EXP") %>% filter(type=="MF") %>%
                           select(goTerm), 
                         goAnnotations.sub2 %>% inner_join(go.goSlim.plant, by=c(goTerm = "GOSlimTerm")) %>% filter(evCode=="EXP") %>% filter(type=="MF") %>%
                           select(goTerm)
                         ) %>% nrow(), 
  category = c("SubGenome 1", "SubGenome 2"), 
  lty = rep("blank", 2), 
